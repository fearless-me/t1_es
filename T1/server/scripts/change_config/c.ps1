...
$MyFile = [System.IO.File]::ReadAllLines($CenterBat, $Utf8NoBomEncoding) -replace $CenterHost_Old, $CenterHost_New; 
[System.IO.File]::WriteAllLines($CenterBat, $MyFile, $Utf8NoBomEncoding); 

#
Write-Host("处理$GameIni");
$MyFile = [System.IO.File]::ReadAllLines($GameIni, $Utf8NoBomEncoding) -replace $CenterOldNode, $CenterNewNode; 
