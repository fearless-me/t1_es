...
info_log(Fmt, Args)-> io:format(user, "~ts", [color:green(io_lib:format(Fmt,Args))]).

warn_log(Fmt)-> warn_log(Fmt, []).
warn_log(Fmt, Args) -> io:format(user, "~ts", [color:yellowb(io_lib:format(Fmt,Args))]).

error_log(Fmt) -> error_log(Fmt, []).
error_log(Fmt, Args)-> io:format(user, "~ts", [color:redb(io_lib:format(Fmt,Args))]).
