...
%%玩家测试期间预充值返钻石
-record(rec_account_recharge_back,{
	accountID = 0,				%%账号ID bigint(20) unsigned
	money = 0.0,				%%充值货币总数 float unsigned
	roleID = 0,				%%锁定的角色ID bigint(20) unsigned
	giftNumber = 0,				%%赠品奖励次数 int(11) unsigned
	updateTime = 0				%%刷新时间 int(11) unsigned
...
%%时装新品折扣活动配置表
-record(rec_activity_new_fashion,{
	id = 0,				%%活动ID int(10) unsigned
	batch = 0,				%%批次号 int(10) unsigned
	fashionID = "",				%%时装ID(区分男女,1男,0女) varchar(50)
	itemID = "",				%%道具盒ID(区分男女) varchar(50)
	discount = 0.0,				%%折扣率 float unsigned
	cointype = 0,				%%货币类型 tinyint(4) unsigned
	original_price = 0,				%%原价 int(11) unsigned
	price = 0,				%%现价 int(11) unsigned
	start_time = 0,				%%开启时间时间戳 int(11) unsigned
	time = 0,				%%持续时间(小时) int(11) unsigned
	resources_bg = "",				%%原画资源 varchar(255)
	fashion_show = 0				%%时装可见性,1可见,0不可见 tinyint(4) unsigned

}).

%%Aruna世界大战
-record(rec_aruna_world_war,{
	roleID = 0,				%% bigint(20) unsigned
	contribution = 0,				%%个人贡献值 int(11) unsigned
	sample_point = 0,				%%样本值 int(11) unsigned
	get_world_reward_phase = 0,				%%领取世界奖励的阶段 tinyint(3) unsigned
	last_contribution = 0,				%%上一次贡献值 int(11) unsigned
	last_rank = 0,				%%上一次的排名 int(11) unsigned
	get_reward_time = 0,				%%上一次排名结算后，领取奖励时间 int(11) unsigned
	task_fresh_time = 0				%%任务刷新时间 int(11) unsigned
...
-record(rec_base_role,{
	...
	level = 0,				%%玩家的当前等级 smallint(5) unsigned
...
-record(rec_buff,{
	...
	level = 0				%%buff等级 smallint(5) unsigned
...
-record(rec_business,{
	...
	name = "",				%%活动内容 varchar(50)
	des = "",				%%活动描述 varchar(512)
	pic = "",				%%图片MD5 varchar(128)
	picType = 0,				%%0默认图片，1上传图片 tinyint(4) unsigned
	activityType = 0,				%%活动类型，1首冲，2成长基金激活，3成长基金领取，4累冲，5消费返利，6现金礼包，7每日充值，8首登现金特卖 tinyint(4) unsigned
	isAlonePay = 0,				%%是否单独计充值 tinyint(3) unsigned
	resetClaimNumType = 0,				%%重置次数，1按天重置，2按周重置，3按月重置 tinyint(4) unsigned
	label = 0,				%%标签，1稀有，2热卖 tinyint(4) unsigned
	claimMoney = 0.0,				%%需要真实货币 float unsigned
	claimGold = 0,				%%需要钻石 int(11) unsigned
	claimConsumeType = 0,				%%消费金额类型，限定3和6，即只记录绑定钻石或者钻石 tinyint(4) unsigned
	claimConsume = 0,				%%消费金额 int(11) unsigned
	claimNum = 0,				%%购买次数限制 int(11) unsigned
	claimIsBuy = 0,				%%开服基金是否购买 tinyint(4) unsigned
	claimVip = 0,				%%特权卡限制 tinyint(4) unsigned
	claimLevel = 0,				%%等级需求 smallint(6) unsigned
	rewardCoinType = 0,				%%奖励货币类型 tinyint(4) unsigned
	rewardCoinNum = 0,				%%奖励货币数量 int(11) unsigned
	rewardPackageID = 0,				%%奖励 int(11) unsigned
	item1 = 0,				%%奖励道具ID int(11) unsigned
	num1 = 0,				%%奖励道具个数 int(11) unsigned
	item2 = 0,				%%奖励道具ID int(11) unsigned
	num2 = 0,				%%奖励道具个数 int(11) unsigned
	item3 = 0,				%%奖励道具ID int(11) unsigned
	num3 = 0,				%%奖励道具个数 int(11) unsigned
	item4 = 0,				%%奖励道具ID int(11) unsigned
	num4 = 0,				%%奖励道具个数 int(11) unsigned
	item5 = 0,				%%奖励道具ID int(11) unsigned
	num5 = 0,				%%奖励道具个数 int(11) unsigned
	item6 = 0,				%%奖励道具ID int(11) unsigned
	num6 = 0,				%%奖励道具个数 int(11) unsigned
	item7 = 0,				%%奖励道具ID int(11) unsigned
	num7 = 0,				%%奖励道具个数 int(11) unsigned
	item8 = 0,				%%奖励道具ID int(11) unsigned
	num8 = 0,				%%奖励道具个数 int(11) unsigned
	item9 = 0,				%%奖励道具ID int(11) unsigned
	num9 = 0,				%%奖励道具个数 int(11) unsigned
	item10 = 0,				%%奖励道具ID int(11) unsigned
	num10 = 0,				%%奖励道具个数 int(11) unsigned
	startTime = 0,				%%开始时间 int(11) unsigned
	endTime = 0,				%%结束时间 int(11) unsigned
	addTime = 0,				%%添加时间 int(11) unsigned
	platformItemID = "",				%%平台订单映射ID varchar(255)
	giveGold = 0,				%%额外赠送的钻石数 int(11) unsigned
	isAddClaimGold = 0				%%是否计入需要钻石,0:不计入   1:计入' tinyint(3) unsigned

}).

%%集字活动时间和版本号配置持久化表
-record(rec_collection_words,{
	group = 0,				%%群组id tinyint(1)
	ver = 0,				%%活动时间变动(版本号) tinyint(1)
	open_time = 0,				%%开启时间时间戳 int(11)
	limit_time = 0				%%持续时间(小时) int(11)
...
%%副本排行榜
-record(rec_copy_rank,{
	copyMapID = 0,				%% int(11)
	instanceGroup = 0,				%% smallint(6)
	star = 0,				%% smallint(6)
	teamList = ""				%% text
...
%%跨服采集资源争夺战
-record(rec_crossgatherbattle,{
	roleID = 0,				%%角色ID bigint(20) unsigned
	gatherPoint = 0,				%%积分 int(11) unsigned
	remainTime = 0,				%%剩余标准时间，单位秒 int(11) unsigned
	resetTime = 0				%%下次重置时间 int(11) unsigned
...
-record(rec_db_info,{
	...
	serverStatus = 0				%%服务器状态，0正常，允许所有人进入；1维护，只允许白名单进入 tinyint(3) unsigned
...
-record(rec_equip_refine_info,{
	...
	refineBless = 0,				%%精炼的祝福值 smallint(6)
	level = 0,				%%精炼等级 tinyint(4) unsigned
	prog = 0,				%%装备部位冲星进度等级(之前升星的步数，已经废弃了，为0) tinyint(4)
...
%%跨服好友找回表
-record(rec_friend2_get_back,{
	roleID = 0,				%%角色id bigint(20) unsigned
	tarRoleID = 0				%%合服后需要找回的跨服好友角色ID bigint(20) unsigned
...
%%团购活动商品表
-record(rec_group_buy_goods,{
	id = 0,				%%商品id(商品包的ID,与item无关,仅做区分使用) int(10) unsigned
	group = 0,				%%群组id int(10) unsigned
	collect_num = 0,				%%限购数 int(10) unsigned
	product_name = "",				%%商品名称(直接读取使用) varchar(50)
	product_icon = "",				%%商品ICON路径 varchar(255)
	product_quality = 0,				%%品质框 tinyint(3) unsigned
	price = 0,				%%原价,仅显示 bigint(20) unsigned
	true_price = 0,				%%实际默认价格 bigint(20) unsigned
	rebate1_num = 0,				%%打折需要数量1 bigint(20) unsigned
	rebate1_price = 100,				%%第1次折扣(100~1) tinyint(3) unsigned
	rebate2_num = 0,				%%打折需要数量2 bigint(20) unsigned
	rebate2_price = 100,				%%第2次折扣(100~1) tinyint(3) unsigned
	rebate3_num = 0,				%%打折需要数量3 bigint(20) unsigned
	rebate3_price = 100,				%%第3次折扣(100~1) tinyint(3) unsigned
	rebate4_num = 0,				%%打折需要数量4 bigint(20) unsigned
	rebate4_price = 100,				%%第4次折扣(100~1) tinyint(3) unsigned
	rebate5_num = 0,				%%打折需要数量5 bigint(20) unsigned
	rebate5_price = 100,				%%第5次折扣(100~1) tinyint(3) unsigned
	item1_id = 0,				%%道具1的id smallint(5) unsigned
	item1_num = 0,				%%道具1数量 bigint(20) unsigned
	item2_id = 0,				%%道具2的id smallint(5) unsigned
	item2_num = 0,				%%道具2数量 bigint(20) unsigned
	item3_id = 0,				%%道具3的id smallint(5) unsigned
	item3_num = 0,				%%道具3数量 bigint(20) unsigned
	item4_id = 0,				%%道具4的id smallint(5) unsigned
	item4_num = 0,				%%道具4数量 bigint(20) unsigned
	item5_id = 0,				%%道具5的id smallint(5) unsigned
	item5_num = 0,				%%道具5数量 bigint(20) unsigned
	score = 0,				%%购买商品可累计的积分 bigint(20) unsigned
	buy_count = 0				%%已购买总数量(做全服能量值使用) -> 对应商品id的购买数量 bigint(20) unsigned

}).

%%团购活动积分奖励商品信息
-record(rec_group_buy_reward,{
	id = 0,				%%奖励id int(10) unsigned
	group = 0,				%%群组id int(10) unsigned
	score = 0,				%%积分对应 bigint(20) unsigned
	reward1_id = 0,				%%奖励道具1id smallint(5) unsigned
	reward1_num = 0,				%%道具1的数量 smallint(20) unsigned
	reward2_id = 0,				%%奖励道具2id smallint(5) unsigned
	reward2_num = 0,				%%道具2的数量 smallint(20) unsigned
	reward3_id = 0,				%%奖励道具3id smallint(5) unsigned
	reward3_num = 0,				%%道具3的数量 smallint(20) unsigned
	reward4_id = 0,				%%奖励道具4id smallint(5) unsigned
	reward4_num = 0,				%%道具4的数量 smallint(20) unsigned
	reward5_id = 0,				%%奖励道具5id smallint(5) unsigned
	reward5_num = 0				%%道具5的数量 smallint(20) unsigned

}).

%%团购活动状态
-record(rec_group_buy_state,{
	ver = 0,				%%版本 int(10) unsigned
	group = 0,				%%群组ID int(10) unsigned
	image = "",				%%图片路径(客户端背景图) varchar(255)
	open_time = 0,				%%活动开启时间 int(10)
	limit_time = 0,				%%持续时间(小时) int(10) unsigned
	score_add_time = 0,				%%积分领取延后时间(小时) int(10) unsigned
	clear_time = 0				%%小于该时间前产生的积分需要被清空LUNA-7248 int(10) unsigned
...
-record(rec_guild,{
	...
	worldBossAwardInfo = []				%%该家族领取世界boss奖励信息 varchar(128)
...
%%记录家族Bos每阶最快杀死时间
-record(rec_guild_bosskill_rank,{
	guildID = 0,				%%家族ID bigint(20) unsigned
	guildBossLevel = 0,				%%家族打的BOSS阶级 tinyint(3) unsigned
	useTime = 0				%%使用时间 int(11) unsigned
...
%%家族扩展数据
-record(rec_guild_ext,{
	guildID = 0,				%%家族ID bigint(20) unsigned
	canRename = 0				%%是否允许改名，0为不允许，1为允许 tinyint(3) unsigned
...
-record(rec_guild_member,{
	...
	tTaskID = 0,				%%正在索取的任务ID smallint(5) unsigned
	tGoodsID = 0,				%%正在索取的道具或装备ID smallint(5) unsigned
	tGoodsM = 0,				%%正在索取的道具或装备M值（已捐赠数量） smallint(5) unsigned
	tGoodsN = 0,				%%正在索取的道具或装备N值（最大捐赠数量） smallint(5) unsigned
	tGoodsTime = 0				%%发布索取的时间，用于服务端启动时重置陈旧数据 int(11) unsigned
...
%%家族精英联赛初赛决赛排名表
-record(rec_guild_war_rank,{
	guildID = 0,				%% bigint(20) unsigned
	rank = 0				%%家族精英联赛初赛决赛排名 smallint(5) unsigned
...
-record(rec_home,{
	...
	upgradeHomeCD = 0				%%家园升级到期时间 int(11) unsigned
...
%%家园家具数据
-record(rec_home_furnitrue,{
	roleID = 0,				%%玩家ID bigint(20)
	furniTrueData				%%玩家拥有的家具信息：家具UID，购买时间，itemID blob
...
%%韩国naver论坛奖励独立需求
-record(rec_koreanaver,{
	id = 0,				%%活动ID tinyint(4)
	name = "",				%%活动名 varchar(50)
	beginTime = 0,				%%活动开始时间 int(4)
	endTime = 0,				%%活动结束时间 int(4)
	itemID = 0,				%%奖励道具ID int(4)
	itemCount = 0,				%%奖励道具数量 int(4)
	mailTitle = "",				%%发送奖励邮件的标题 varchar(128)
	mailContent = "",				%%发送奖励邮件的内容 varchar(255)
	param = "",				%%活动参数 varchar(255)
	ver = 0,				%%版本号 int(4)
	opUser = "",				%%操作者 varchar(128)
	id_num = 0,				%%活动序号 int(4)
...
-record(rec_login_award_conf,{
	...
	is_before = 0,				%%是否仅发放活动开始前已创建的角色 0为否;1为是 tinyint(1) unsigned
	level_min = 0,				%%最小等级 smallint(5) unsigned
	level_max = 0,				%%最高等级 smallint(5) unsigned
	...
	sex = 0,				%%性别限制，0为不限制 tinyint(3) unsigned
	career = 0				%%职业限制，0为不限制 smallint(5) unsigned
...
-record(rec_mall,{
	...
	level = 0,				%%可见最小等级限制 smallint(5) unsigned
	levelMax = 105,				%%可见最大等级限制 smallint(5) unsigned
	...
	buyNumDefalut = 1,				%%默认购买数量 smallint(6) unsigned
...
%%婚姻婚礼
-record(rec_marriage_wedding,{
	roleID = 0,				%%预约人ID bigint(20) unsigned
	targetRoleID = 0,				%%另一半ID bigint(20) unsigned
	weddingLevel = 0,				%%婚礼档次 int(11) unsigned
	startTime = 0,				%%开始时间 bigint(20) unsigned
	weddingState = 0,				%%婚礼状态 int(11) unsigned
	allGifts = 0,				%%非绑礼金 int(11) unsigned
	inviteListInfo,				%%请帖信息 blob
	bindGifts = 0				%%绑定礼金 int(11) unsigned
...
%%玩家保存数据
-record(rec_old_player_rebate,{
	roleID = 0,				%%角色ID bigint(20) unsigned
	diamond_num = 0,				%%钻石返利基数 bigint(20) unsigned
	notbinddiamond_num = 0				%%星月币返利基数 bigint(20) unsigned
...
-record(rec_operate_activity,{
	...
	minlevel = 0,				%%活动参与最小等级 smallint(5) unsigned
	maxlevel = 0,				%%活动参与最大等级 smallint(5) unsigned
	arg1 = "",				%%活动参数 text
	arg2 = "",				%%活动参数 text
	arg3 = "",				%%活动参数 text
	arg4 = "",				%%活动参数 text
...
%%游戏图片链接公告
-record(rec_picture_notice,{
	site = 0,				%%图片位置 smallint(5) unsigned
	ver = 0,				%%图片版本 bigint(20) unsigned
	source = "",				%%图片资源链接 varchar(255)
	target = ""				%%图片指向链接 varchar(255)
...
%%头像框领取情况表
-record(rec_player_portrait_frame,{
	roleID = 0,				%%角色ID bigint(20) unsigned
	frameID = 0,				%%头像框ID smallint(6) unsigned
	time = 0				%%头像框领取时间 int(16)
...
%%玩家举报信息表
-record(rec_player_report,{
	accountID = 0,				%%举报者账号ID bigint(20) unsigned
	roleID = 0,				%%举报者角色ID bigint(20) unsigned
	roleName = "",				%%举报者角色名 varchar(32)
	beAccountID = 0,				%%被举报者账号ID bigint(20) unsigned
	beRoleID = 0,				%%被举报者角色ID bigint(20) unsigned
	beRoleName = "",				%%被举报者角色名 varchar(32)
	reportTime = 0,				%%举报时间 int(11) unsigned
	endTime = 0				%%封禁结束时间 int(11) unsigned
...
-record(rec_recharge,{
	...
	businessID = 0				%%活动ID int(11) unsigned
...
-record(rec_recharge_bak,{
	...
	item_id,				%%购买道具的id,次ID为渠道-平台-游戏三方约定的一个发货代码，因此对应到的具体物品id需要一个映射关系、订单系统暂时不对该功能进行支持。扩展的时候只需要增加一个配置文件将对应关系配置起来即可。 varbinary(255)
	category,				%%订单类型 varbinary(255)
	businessID = 0				%%活动ID int(10) unsigned
...
-record(rec_rune_base,{
	...
	subType = 0,				%%穿戴部位 smallint(5) unsigned
