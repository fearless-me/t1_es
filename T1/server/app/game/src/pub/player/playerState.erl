...
setOffLineFlag(Type) when Type =:= ?OffLineTypeRobKickOut orelse Type =:= ?OffLineTypeGmKickOut orelse
	...
	ok;
setOffLineFlag(Type) ->
	?ERROR("setOffLineFlag badarg:[~p]",[Type]),
	ok.
...
setActionStatus(ActionStatus) when ActionStatus > 0, ActionStatus =< ?CreatureActionStatusMax ->
	case getActionStatus() of
		?CreatureActionStatusDead ->
			?ERROR("Player[~ts] Dead, can not set ActStatus[~p],Stack[~p]",
				[playerState:getName(),
					ActionStatus,
					misc:getStackTrace()
				]);
...
setStandActionStatusFromDeadStatus() ->
	case getActionStatus() of
		...
		Status ->
			?ERROR("Player[~ts] Status[~p] is not Dead Status,,Stack[~p]",
				[playerState:getName(),
					Status,
					misc:getStackTrace()
				])
...
getMapIDGroup() ->
	case groupBase:getMapIDByGroupID(getGroupID()) of
		0 ->
			getMapID();
		MapID ->
			MapID
...
setGroupID(GroupID) when erlang:is_integer(GroupID) andalso GroupID >= 0 ->
	...
	case OldGroupID =/= GroupID of
		true ->
			%%?DEBUG("setGroupID: [~p ~s] [~p -> ~p]", [playerState:getRoleID(), playerState:getName(), OldGroupID, GroupID]),
...
setGroupIDOnly(GroupID) ->
	...
	case OldGroupID =/= GroupID of
		true ->
			%%?DEBUG("setGroupIDOnly: [~p ~s] [~p -> ~p]", [playerState:getRoleID(), playerState:getName(), OldGroupID, GroupID]),
			%% 强制脱战
			playerBattle:leaveBattle(getPlayerCode()),
...
-spec setPos(X, Y) -> ok when X :: float(), Y :: float().
...
getOldMapPos() ->
	case get(oldMapPos) of
		{OldMapId, OldX, OldY} ->
			{OldMapId, OldX, OldY};
		Other ->
			?ERROR("getOldMapPos: ~p ~p ~p ~p ~p", [self(), ?MODULE, getPlayerCode(), Other, erlang:get_stacktrace()]),
			{1, 38.3,104.3}
	end.


%%设置当前地图的销毁时间
setCurrentCopyMapDestroyTime(MapID, MapPid) ->
	put(lastCopyMapDestoryTime,{MapID, MapPid}).
%%获取当前地图的销毁时间
getCurrentCopyMapDestroyTime()-> get(lastCopyMapDestoryTime).
...
% 设置角色头
setHead(Head) when erlang:is_integer(Head) ->
	put(playerHead, Head),
	ok.
...
setRace(Race) when erlang:is_integer(Race) ->
	?ERROR("race is error:rid=~p,race=~p",[playerState:getRoleID(), Race]),
	ok.
...
%%保存已经激活的套装
-spec setFashionSuitList(List) -> ok when List::list().
setFashionSuitList(List) ->
	put(fashionSuitList,List),
	ok.

%%获取已经激活的套装
-spec getFashionSuitList() -> [integer()].
getFashionSuitList() ->
	case get(fashionSuitList) of
		undefined ->
			[];
		List ->
			List
...
setCoin(CoinType,CoinNum) when CoinType > 0 andalso CoinType =< ?CoinTypeMax
	andalso erlang:is_number(CoinNum) andalso CoinNum >= 0 ->
	NewCoinNum = erlang:trunc(CoinNum),
	put({coin,CoinType},NewCoinNum),
...
setRebornHp(Hp) when erlang:is_integer(Hp), Hp > 0 ->
	case playerState:getActionStatus() of
		...
		State ->
			...
			?ERROR("setRebornHp name=~ts, roleID=~p, curHP=~p, State=~p",
...
%% 			catch
%% 				_:_ ->
%% 					?ERROR("player[~ts] is dead,can not set hp,Stack[~p]",
%% 					           [playerState:getName(),
...
setCurPhys(Value) when is_integer(Value) ->
	MaxPhys = 0,%% trunc(getBattlePropTotal(?Prop_physical)),
...
% 获取角色真正移动方向
-spec getMoveRealDir() -> Val when Val :: {float(), float()}.
getMoveRealDir() ->
	case get(playerMoveRealDir) of
		undefined ->
			{1.0, 0.0};
		Dir ->
			Dir
...
% 设置角色真正移动方向
-spec setMoveRealDir(Dir) -> OldVal when
	Dir :: {float(), float()}, OldVal :: {float(), float()}.
setMoveRealDir({0.0, 0.0}) ->
	getMoveRealDir();	%% 新朝向无效，保持原朝向
setMoveRealDir({DirX, DirY}) ->
	Len = math:sqrt(DirX * DirX + DirY * DirY),
	case Len >= 0.3333 of
		true ->
			put(playerMoveRealDir, {DirX/Len, DirY/Len});
		_ ->
			getMoveRealDir()	%% 新朝向无效，保持原朝向
	end.
...
-spec getSkill() -> Val | undefined when Val :: [#recSkill{}, ...].
getSkill() ->
	get(skill).

% 设置角色技能列表，Value为#recSkill{}
-spec setSkill(SkillList) -> OldVal | undefined when
	SkillList :: [#recSkill{}, ...], OldVal :: [#recSkill{}, ...].
...
getFinalAttackIntervalTime() ->
	AttackSpeed = 0, %% getBattlePropTotal(?Prop_attackspeed),
...
getBattlePropTotal(Prop) when Prop >= ?Prop_Min andalso Prop =< ?PropMax ->
...
% 获取角色触发器
-spec getTrigger() -> Val | undefined when Val :: array:array().
getTrigger() ->
	get(playerTrigger).

% 设置角色触发器
-spec setTrigger(ArTrigger) -> OldVal | undefined | badarg when
	ArTrigger :: array:array(), OldVal :: array:array().
...
getTeamID() ->
	gsTeamInterface:getTeamID(playerState:getRoleID()).
%% 发送普通队伍队友位置信息的缓存
...
%% 获取召唤骑宠对应地图对象的动作状态
%% 用于离开地图前记录旧有状态，同步到新的地图
-spec makeCallPetActionStatusList() -> ok.
makeCallPetActionStatusList() ->
	Ets = getMapPetEts(),
	case ets:info(Ets, size) of
		undefined ->
			put(callPetActionStatusList, []);
		_ ->
			List =
				[
					case ets:lookup(Ets, Code) of
						[#recMapObject{actionStatus = AS}] ->
							{Code, AS};
						_ ->
							{Code, ?CreatureActionStatusStand}  %% 默认站立
					end || #recCallPet{pet_code = Code} <-  getCallPet()
				],
			put(callPetActionStatusList, List)
	end,
	ok.
-spec callPetActionStatusList() -> list().
callPetActionStatusList() ->
	case get(callPetActionStatusList) of
		undefined ->
			[];
		L ->
			L
...
%%设置玩家当前召唤宠物信息
-spec setCallPet(CallPet :: list()) ->  ok.
setCallPet(CallPet) when erlang:is_list(CallPet) ->
	put(playerCallPet, CallPet),

	%% 更新宠物与召唤宠物
	PlayerCode = getPlayerCode(),
	PlayerEts = getMapPlayerEts(),
	case PlayerEts /= undefined andalso PlayerCode /= undefined of
		true ->
			ets:update_element(PlayerEts, PlayerCode, {#recMapObject.callPetList, CallPet});
		_ ->
			skip
	end,
	ok.
...
%% 获取角色已接受任务
-spec getAcceptedTask() -> Val when Val :: [#rec_task{},...] | [].
getAcceptedTask() ->
...
%% 上次执行传送功能的时间
setVipLastTransmit(Time) ->
	put('VipLastTransmit', Time).
getVipLastTransmit() ->
	case get('VipLastTransmit') of
		undefined ->
			0;
		Time ->
			Time
...
%%获取插槽技能
-spec getSlotSkill() -> [#recSlotSkill{}, ...].
getSlotSkill() ->
...
ringBuffLastTime(RingBuffID) ->
	case get({ringBuffLastTime, RingBuffID}) of
		undefined ->
			0;
		LastTime ->
			LastTime
...
ringBuffLastTime(RingBuffID, BuffTime) ->
	put({ringBuffLastTime, RingBuffID}, BuffTime).
...
setSelfGuildInfo(Info) ->
	{NGuildID, NGuildName, NGuildPower, NGuildLevel} =
		case Info of
			{#rec_guild{guildID = GuildID, guildName = GuildName, guildLevel = GuildLevel}, #rec_guild_member{power = GuildPower}} ->
				{GuildID, GuildName, GuildPower, GuildLevel};
			_ ->
				{0, [], 0}
		end,

	playerPropSync:setInt64(?PubProp_GuildID, NGuildID),
	playerPropSync:setInt(?PubProp_GuildPower, NGuildPower),
	playerPropSync:setString(?PubProp_GuildName, NGuildName),
	playerPropSync:setInt(?PriProp_GuildLevel, NGuildLevel),
	put(selfGuildInfo, Info).

%% 更新个人军团的同步信息
-spec updateSelfGuildInfo() -> ok.
updateSelfGuildInfo() ->
	{GuildID, GuildName, GuildLevelPower, GuildLevelReal} =
		case getSelfGuildMember() of
			#rec_guild_member{guildID = GID, power = Power} ->
				case ets:lookup(rec_guild, GID) of
					[#rec_guild{guildName = GName, guildLevel = GuildLevelReal_}] ->
						{GID, GName, Power, GuildLevelReal_};
					_ ->
						{0, "", 0, 0}
				end;
			_ ->
	...
	playerState:setGuildIDCache(GuildID),
	case playerPropSync:getProp(?PubProp_GuildID) =:= GuildID of
		true -> skip;
		_ -> playerPropSync:setInt64(?PubProp_GuildID, GuildID)
	end,
	case playerPropSync:getProp(?PubProp_GuildPower) =:= GuildLevelPower of
		true -> skip;
		_ -> playerPropSync:setInt(?PubProp_GuildPower, GuildLevelPower)
	end,
	case playerPropSync:getProp(?PubProp_GuildName) =:= GuildName of
		true -> skip;
		_ -> playerPropSync:setString(?PubProp_GuildName, GuildName)
	end,
	case playerPropSync:getProp(?PriProp_GuildLevel) =:= GuildLevelReal of
		true -> skip;
		_ -> playerPropSync:setInt(?PriProp_GuildLevel, GuildLevelReal)
	end,
	ok.

%% 获取自己的军团成员信息
-spec getSelfGuildMember() -> #rec_guild_member{} | {}.
getSelfGuildMember() ->
	RoleID = getRoleID(),
	case ets:lookup(rec_guild_member, RoleID) of
		[#rec_guild_member{guildID = GuildID} = Member] ->
			...
			case ets:lookup(rec_guild, GuildID) of
				...
				_ ->
					%% 不在
					?ERROR("guild is delete:~p,~p", [GuildID, RoleID]),
					psMgr:sendMsg2PS(?PsNameGuild, deleteErrorGuildMember, {GuildID, RoleID}),
					{}
			end;
		_ ->
			{}
...
getGuildID() ->
	IsSync =
		case getGuildIDCache() of
			undefined ->
				true;
			_ ->
				not misc:isInLongTickTimeCD('GuildIDCacheSync', 10000)
		end,
	case IsSync of
		true ->
			GID =
...
getGuildIDCache() ->
	get('GuildIDCache').
setGuildIDCache(GuildID) ->
	put('GuildIDCache', GuildID).
delGuildIDCache() ->
	erlang:erase('GuildIDCache').

%% 获取公会等级
-spec getGuildLevel() -> uint() | 0.
getGuildLevel() ->
	case getSelfGuildInfo() of
		#rec_guild{guildLevel = GuildLevel} -> GuildLevel;
		_ -> 0
	end.
...
getGuildName() ->
	case getSelfGuildInfo() of
		#rec_guild{guildName = GuildName} -> GuildName;
		_ -> ""
	end.
...
%% !!!!!! 调用者注意不要将默认头像框[#recPlayerFrame{frameID = 1, time = 0 }]写入了
%%设置玩家头像框列表
-spec setPlayerFrameList(List::list()) -> ok.
setPlayerFrameList(List) ->
	put(playerFrame, List),
	ok.

%%获取玩家头像框列表
-spec getPlayerFrameList() -> list().
getPlayerFrameList() ->
	case get(playerFrame) of
		undefined ->
			[];
		List ->
			List
...
%%获取玩家头像框列表(带默认头像框)
-spec getPlayerFrameListWithDefault() -> list().
getPlayerFrameListWithDefault() ->
	case get(playerFrame) of
		undefined ->
			[#recPlayerFrame{frameID = 1, time = 0 }];
		List ->
			%%每个玩家增加一个默认头像框
			[#recPlayerFrame{frameID = 1, time = 0 }] ++ List
	end.
...
setPlayerSysForce(SysID, Force) ->
	?DEBUG("player [~ts] sys [~p] force value:~p", [ getName(), SysID, Force]),
...
setPetSysForce(SysID, Force) ->
	?DEBUG("player [~ts] pet sys [~p] force value:~p", [getName(), SysID, Force]),
...
getLiveness()->
	case get(liveness) of
		undefined->
			?ERROR("player:getLiveness,stack= ~p.",[misc:getStackTrace()]),
			{};
		L -> L
	end.
...
setReturnCallBack(Tuple)->
	put(returnCallBack, Tuple).
...
setSkipSkillCheck(Flag)->
	put('SkipSkillChech', Flag).

getSkipSkillCheck()->
	case get('SkipSkillChech') of
		undefined->
			false;
		V -> V
	end.

setPlayerGuildID(Guild)->
	put('setPlayerGuildID', Guild).
...
setPlayerEnterMapAddBuffInfo(BuffInfo)->
	put('enterMapIsNeedAddBuffInfo', BuffInfo).
getPlayerEnterMapAddBuffInfo() ->
	get('enterMapIsNeedAddBuffInfo').

setPlayerMysteriousBuyTime(ThisTime) ->
	put('playerMysteriousBuyTime', ThisTime).
getPlayerMysteriousBuyTime() ->
	case get('playerMysteriousBuyTime') of
		undefined ->
			0;
		Old ->
			Old
...
%%缓存添加目标队列
-spec setCrossAddFriend(TargetIDs::uint64())->ok.
setCrossAddFriend(TargetIDs)->
	put(crossAddPairs,TargetIDs).
%%获取缓存的好友添加信息
-spec getCrossAddFriend() -> list().
getCrossAddFriend() ->
	case get(crossAddPairs) of
		undefined ->[];
...
%%跨服中缓存添加好友的回归回调数据
setAddFriendCallBack(Tuple)->
	put(returnAddFriendCallBack, Tuple).
getAddFriendCallBack() ->
	get(returnAddFriendCallBack).