...
getGlobalCfg(GlobalCfg) ->
	case getCfg:getCfgPStack(cfg_globalsetup, GlobalCfg) of
		#globalsetupCfg{setpara = [SetPara]} ->
			SetPara;
		[] ->
			?ERROR("not found the cfg [~p]", [GlobalCfg]),
			not_found
	end.
...
getWingRegeneratio() ->
	case getGlobalCfgList(wing_regeneratio) of
		undefined ->  [];
		L -> L
	end.
...
getTransformationBuff() ->
	case getGlobalCfgList(distortion_big_and_small) of
		[Big, Small] -> [erlang:tuple_to_list(Big), erlang:tuple_to_list(Small)];
		_ -> [[], []]
	end.
