...
filterTarget(SkillID, MyTeam, KillList, SelfObject, TargetList) ->
	%% 为了提高效率，优先过滤个数，再过滤重复，再过滤其它条件
	%% 因为skill.targetSearch设计问题，需预先判断目标阵营再过滤个数，否则可能有技能释放无效的情况
	TargetList2 = relationShipLimitFilter(SkillID, MyTeam, KillList, SelfObject, TargetList),
	TargetList3 = countLimitFilter(SkillID, TargetList2),
	TargetList4 = repeatFilter(TargetList3),
	Fun =
		fun(TargetObject) ->
			case isFilter(SkillID, MyTeam, KillList, SelfObject, TargetObject, false) of
				false ->
					true;
				_Error ->
					false
...
relationShipLimitFilter(SkillID, MyTeam, KillList, SelfObject, ObjectList) ->
	#skillCfg{targetSearch = SearchType} = getCfg:getCfgPStack(cfg_skill, SkillID),
	F =
		fun(TargetObject) ->
			checkRelationShip(SearchType, MyTeam, KillList, SelfObject, TargetObject)
		end,
	lists:filter(F, ObjectList).
...
castDistFilter(SkillID, SelfObject, TargetObject) ->
	#skillCfg{ranger = Ranger, aoeRange = AoeRange, aoe = IsAoe} = getCfg:getCfgPStack(cfg_skill, SkillID),
	Distance =
		case IsAoe of
			0 -> Ranger;
			_ -> Ranger + AoeRange
...
%% 							Dist1 = math:sqrt(SQR),
%% 							?ERROR("Player[~p] at Pos[~p,~p] Attack TargetCode[~p]Pos[~p,~p]Dist[~p], Ranger [~p], SkillID [~p]",
...
groupFilter(SelfGroudID, TargetGroudID) ->
	groupBase:canSeeTarget(SelfGroudID, TargetGroudID).
...
%%获取PvP阵营关系
-spec getPvpCampRelation(MyCamp, CfgCamp, IsMyTreamer, IsSameGuild, IsRedName, IsCanKill) -> ?Camp_Neutral | ?Camp_Friendly | ?Camp_Hostile when
	MyCamp :: uint(),
	CfgCamp :: #factionListCfg{},
	IsMyTreamer :: boolean(),
	IsSameGuild :: boolean(),
	IsRedName :: boolean(),
	IsCanKill :: boolean().
getPvpCampRelation(?CampPeace, CfgCamp, true = _IsMyTreamer, _IsSameGuild, _IsRedName, _IsCanKill) ->
	CfgCamp#factionListCfg.teamMate;
getPvpCampRelation(?CampPeace, _CfgCamp, _IsMyTreamer, _IsSameGuild, true = _IsRedName, _IsCanKill) ->
	?Camp_Hostile;	%% LUNA-6421 【主需求】和平模式需要可攻击红名玩家
getPvpCampRelation(?CampPeace, _CfgCamp, _IsMyTreamer, _IsSameGuild, _IsRedName, true = _IsCanKill) ->
	?Camp_Hostile;	%% LUNA-6421 【主需求】和平模式需要可攻击红名玩家 （策划追加反击需求）
getPvpCampRelation(?CampPeace, CfgCamp, _IsMyTreamer, true = _IsSameGuild, _IsRedName, _IsCanKill) ->
	CfgCamp#factionListCfg.memberShip;
getPvpCampRelation(?CampPeace, CfgCamp, _IsMyTreamer, _IsSameGuild, _IsRedName, _IsCanKill) ->
	CfgCamp#factionListCfg.nonMemberShip;
getPvpCampRelation(?CampGeneralKill, CfgCamp, true = _IsMyTreamer, _IsSameGuild, _IsRedName, _IsCanKill) ->
	CfgCamp#factionListCfg.teamMate;
getPvpCampRelation(?CampGeneralKill, CfgCamp, _IsMyTreamer, _IsSameGuild, _IsRedName, _IsCanKill) ->
	CfgCamp#factionListCfg.nonMemberShip;
getPvpCampRelation(?CampGuildKill, CfgCamp, true = _IsMyTreamer, _IsSameGuild, _IsRedName, _IsCanKill) ->
	CfgCamp#factionListCfg.teamMate;
getPvpCampRelation(?CampGuildKill, CfgCamp, _IsMyTreamer, true = _IsSameGuild, _IsRedName, _IsCanKill) ->
	CfgCamp#factionListCfg.memberShip;
getPvpCampRelation(?CampGuildKill, CfgCamp, _IsMyTreamer, _IsSameGuild, _IsRedName, _IsCanKill) ->
	CfgCamp#factionListCfg.nonMemberShip.
...
isMyTeamer(TargetType, TargetCode, MyCode, MyTeam) ->
	case TargetType =:= ?ObjTypePlayer orelse TargetType =:= ?ObjTypePet of
		true ->
			case MyTeam of
				#recTeamInfoGS{memberList = Members} ->
...
isMyTeamer([], MyCode, TargetCode) ->
	TargetCode =:= MyCode;
isMyTeamer([#recTeamMemberInfoGS{code = Code} | Members], MyCode, TargetCode) ->
...
isEnemy(pvp, _MyCamp, _TargetCamp, PkMode, IsMyTeamer, IsSameGuild, _IsMyCasterOrSelf, IsRedName, IsCanKill) ->
	...
	CfgCamp = getCfg:getCfgByArgs(cfg_factionList, MyCamp),
	case getPvpCampRelation(MyCamp, CfgCamp, IsMyTeamer, IsSameGuild, IsRedName, IsCanKill) of
		?Camp_Hostile ->
			true;
		_ ->
			false
...
checkRelationShip(?MutiSkillCirMyTeam, MyTeam, KillList, #recMapObject{} = Object, #recMapObject{} = TargetObject) ->
	relationShip(true, MyTeam, KillList, Object, TargetObject);

checkRelationShip(_, MyTeam, KillList, #recMapObject{} = Object, #recMapObject{} = TargetObject) ->
	relationShip(false, MyTeam, KillList, Object, TargetObject);

checkRelationShip(_, _, _, _, _) ->
	false.
...
getCasterCode(MyCode, []) ->
	MyCode;
% 这个是获取宠物的宿主Code
getCasterCode(_, [{CasterCode, _pet_rawLvl}]) ->
	CasterCode;
