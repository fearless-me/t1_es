...
%%增加装备消息--交易行数据使用,直接組裝屬性數據
-spec makeQueryEquipPropInfoList(Equip) -> undefined | [#pk_QueryEquipPropInfo{}] when Equip::#recSaveEquip{}.
makeQueryEquipPropInfoList(#recSaveEquip{
	baseProp = BaseProp,
	enhanceProp = EnhanceProp
}) ->
	EquipBasePropList = makeEquipPropNetMessage(BaseProp),
	EquipEncPropList = makeEquipPropNetMessage(EnhanceProp),
	Fun = fun(#pk_EquipPropInfo{propType = PropType, propKey = PropKey, propAffix = PropAffix, calcType = CalcType, propValue = PropValue}) ->
		#pk_QueryEquipPropInfo{propType = PropType, propKey = PropKey, propAffix = PropAffix, calcType = CalcType, propValue = PropValue} end,
	lists:map(Fun,EquipBasePropList ++ EquipEncPropList).
...
getGoodsCfg(GoodsID) ->
	if
		...
		true ->
			?ERROR("Error GoodsID[~p] in ~p:getGoodsCfg",[GoodsID,?MODULE]),
