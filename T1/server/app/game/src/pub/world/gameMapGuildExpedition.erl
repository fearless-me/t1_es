...
%%
    ...
    ?INFO("~p createStatue,mapid=~p,mapPid=~p,npc=~p",[?MODULE, mapState:getMapId(), self(),NpcInstance]);
...
doCreateBanner(PointIndex, GuildName) ->
    ...
    mapBase:npcSpawn(NpcInstance),
    ?INFO("~p doCreateBanner,mapid=~p,mapPid=~p,npc=~p", [?MODULE, mapState:getMapId(), self(), NpcInstance]),
    ok.
%%-----------------------------------------------------------------------------------------------------------------------
%%-----------------------------------------------------------------------------------------------------------------------
%% 获取旗帜NPC的ID
