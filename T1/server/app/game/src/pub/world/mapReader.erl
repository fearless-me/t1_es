...
%读取所有地图信息
-spec loadAllMaps() -> {ok,array:array()} | {false,Why} when
		  Why :: string().
loadAllMaps() ->
	ArCfg = array:new(),
	...
	case getCfg:getAllCfg(cfg_mapsetting) of
		...
		MapSettingCfgList->
			...
			%%?INFO("MapSettingCfgList ~p",[MapSettingCfgList]),
			ok
	end,
    ...
    case RetInfo of
        {ok,{MapID,Info}} ->
			...
			case erlang:length(WayPtList) =< 0 of
				true ->
					?ERROR("地图[~p]没有路点信息，无法设置地图初始点",[MapID]),
        ...
        OtherInfo ->
			?ERROR("Read MapID[~p] File[~p] Error~p",[MapID,FileName, OtherInfo]),
            throw(OtherInfo)
    end.
...
loadMapFromFile(MapID,Filename) when erlang:is_list(Filename)->
	?INFO("Load Map[ID:~p] File ~p",[MapID,Filename]),
	...
	case Ret of
		{ok, Bin} ->
				...
				{ok,Cfg};
		Error ->
			?ERROR("Load Map[ID:~p] File[~ts] Failed",[MapID,Filename]),
			Error
	end.
...
loadMapFromBinary(MapID,Bin) when erlang:is_integer(MapID) andalso MapID > 0 andalso erlang:is_binary(Bin)->
	...
	case Tag of
		...
		_ ->
			...
			?ERROR("Error Cfg of Map[~p] PathFile",[MapID]),
...
loadMapByVersion(1,MapID,Bin,?GameMapBinFile)->
	...
	#recGameMapCfg{
				...
				colGridNum = misc:ceil(TotalColCellNum / mapView:maxGridCellColNum(false)),
				rowGridNum = misc:ceil(TotalColCellNum / mapView:maxGridCellRowNum(false)),
				block = BlockBin,
				height = HeightBin
	}.

%从二进制数据中读取地图物件信息
-spec loadMapInfoFromBinary(Bin) -> array:array() when
          Bin :: binary().
loadMapInfoFromBinary(Bin) ->
	...
	case Tag of
		...
		_ ->
			%%不是MapInfo文件，策划配置填错了
			?ERROR("Error Cfg of Map MapInfo File"),
