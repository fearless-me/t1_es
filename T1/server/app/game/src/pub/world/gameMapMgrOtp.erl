...
handle_info({alreadyConnectDBServer, _Pid, _Msg}, State) ->
	?INFO("gameMapMgrOtp:~p connectdbserver", [self()]),
	{noreply, State};
...
initGameMapMgr() ->
	...
	case getCfg:getCfgPStack(cfg_mapsetting, MapID) of
		#mapsettingCfg{} ->
			skip;
		_ ->
			?ERROR("Can not find StartMapID[~p] in mapsetting cfg",[MapID]),
...
startMgrWorkOtp(Name,N) ->
	...
	Fun = fun(I,AccIn) ->
		case gameMapMgrWorkerOtp:start_link(Name,I) of
			{'ok', Pid} ->
				?DEBUG("start MapMgr work otp:~p",[Pid]),
				[{Pid} | AccIn];
			Reason ->
				?DEBUG("start MapMgr work otp failed[~p]",[Reason]),
				AccIn
		end
	end,
...
getWorkOtpList() ->
	case get(workOtpList) of
		undefined ->
			?ERROR("Please set Work Otp List first"),
			[];
		List ->
			List
	end.
