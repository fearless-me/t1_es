...
doBackGoddessHp(MonsterCode) when erlang:is_number(MonsterCode) andalso MonsterCode >0 ->
	...
	?DEBUG("zzc==========backupHP new[~w],old[~w]",[NewHp,Hp]),
...
goddessBeKilled(MonsterID,AttackerCode) ->
	...
	if
		MonsterID =:= GoddID ->
			?INFO("goddessBeKilled MapPid:~p attackerCode:~p",[self(),AttackerCode]),
...
goddessSettlement() ->
	...
	Fun = fun(#recMapObject{pid=PlayerPid}) ->
				  psMgr:sendMsg2PS(PlayerPid, goddessSettlement,{0, DiffTime, mapState:getMapId()})
		  end,
	gameMapLogic:doFun4AllPlayer(Fun).
...
initSchedule() ->
	...
	case getCfg:getCfgByArgs(cfg_goddess, Cnf#copyMapDemonBattleCnf.fableCurrentSchedule) of
		#goddessCfg{
			...
			?DEBUG("zzc===========fableMapLevelCoefficient==~p",[Cnf#copyMapDemonBattleCnf.fableMapLevelCoefficient]),
...
getMonsterRec(MonsterLevel,IDList,CD,MonsterNumber,Order) ->
	...
	?DEBUG("times==================~p level[~w]",[CD*Order,MonsterLevel]),
	case R of
		[#recMapObjData{} | _] ->
