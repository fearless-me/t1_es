...
-spec check(Condition::list(), OriginalParams::term(), CEParams::map()) -> boolean().
check([], _OriginalParams, _CEParams) ->
    true;
check(Condition, OriginalParams, CEParams) ->
    RealParams = filter_params(Condition, OriginalParams),
    check_action(Condition, RealParams, CEParams).
...
check_action([_, ?CONDITION_PLAYER_PROP, PropID, ?TARGET_SELF, Logic, Value],
    [#m_object_rw{battle_props = Props}, _], _CEParams) ->
    condition_action_logic:condition_player_prop(Props, PropID, Logic, Value);
check_action([_, ?CONDITION_PLAYER_PROP, PropID, ?TARGET_OTHER, Logic, Value],
    [_, #m_object_rw{battle_props = Props}], _CEParams) ->
    condition_action_logic:condition_player_prop(Props, PropID, Logic, Value);

%%-------------------------------------------------------------------
%% [逻辑关系，BUFF是否存在类型，BUFF区分类型效果，类型参数，是否存在，对象]
check_action([_, ?CONDITION_BUFF_HAS, Cbh, BuffID, ExistOr, ?TARGET_SELF],
    [#m_object_rw{uid = Uid}, _], _CEParams) ->
...
check_action([_, ?CONDITION_BUFF_HAS, Cbh, BuffID, ExistOr, ?TARGET_OTHER],
    [_, #m_object_rw{uid = Uid}], _CEParams) ->
    condition_action_logic:condition_buff_has(Uid, Cbh, BuffID, ExistOr);

%%-------------------------------------------------------------------
check_action(Condition, Params, CEParams) ->
    ?WARN("no realize condition:~p, param:~p, CEParams:~p", [Condition, Params, CEParams]),
    false.
