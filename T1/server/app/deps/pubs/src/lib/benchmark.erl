...
-define(print_title(), io:format("~9s~16s\t~s~n", ["Operation", "Total(us)", "Average(us)"])).
bench(Reps) ->
    io:format("Base Case:~n"),
    ?print_title(),
    print(base_case(Reps)),
%%    io:format("~nNaive Orddict:~n"),
%%    io:format("Operation\tTotal\tAverage~n"),
...
%%    print(smart_orddict(Reps)),
    ...
    io:format("~nNaive maps:~n"),
    ?print_title(),
    print(naive_maps(Reps)),
    io:format("~nNaive ets:~n"),
    ?print_title(),
    print(naive_ets(Reps)),
    io:format("~nSmart ets:~n"),
    ?print_title(),
    print(smart_ets(Reps)),
    io:format("~nSmart lists:~n"),
    ?print_title(),
    print(smart_lists(Reps)),
    io:format("~nNaive lists:~n"),
    ?print_title(),
    print(naive_lists(Reps)),
    ok.
...
print([{Op, Total, Avg} | Rest]) ->
    io:format("~9s~15w\t~.6f~n", [Op, Total, Avg]),
    print(Rest).
...
%% lists benchmark.
naive_lists(Reps) when Reps > 10000 ->
    io:format("disable ~p ~n", [?FUNCTION_NAME]),
    [];
naive_lists(Reps) ->
    benchmark(Reps,
        [],
        fun(K, V, List) -> [{K, V} | List] end,
        fun(K, List) -> lists:keyfind(K, 1, List) end,
        fun(K, V, List) -> lists:keystore(K, 1, List, {K, V}) end,
        fun(K, List) -> lists:keydelete(K, 1, List) end).

%% lists benchmark.
smart_lists(Reps) when Reps > 10000 ->
    io:format("disable ~p ~n", [?FUNCTION_NAME]),
    [];
smart_lists(Reps) ->
    benchmark(Reps,
        [],
        fun(K, _V, List) -> [K | List] end,
        fun({_, _, K}, List) -> lists:keyfind(K, 3, List) end,
        fun({_, _, K} = V, _, List) -> lists:keystore(K, 3, List, V) end,
        fun({_, _, K}, List) -> lists:keydelete(K, 3, List) end).

%% ets benchmark.
naive_ets(Reps) ->
    TabId = ets:new(ets, [{keypos, 3}]),
    Res = benchmark(Reps,
        TabId,
        fun(V, _, _) -> ets:insert(TabId, V) end,
        fun({_, _, K}, _) -> ets:lookup(TabId, K) end,
        fun({_, _, _K} = V, _, _) -> ets:insert(TabId, V) end,
        fun({_, _, K}, _) -> ets:delete(TabId, K) end),
    ets:delete(TabId),
    Res.

%% ets benchmark.
smart_ets(Reps) ->
    TabId = ets:new(ets, [{keypos, 3}, {read_concurrency, true}, {write_concurrency, true}]),
    Res = benchmark(Reps,
        TabId,
        fun(V, _, _) -> ets:insert(TabId, V) end,
        fun({_, _, K}, _) -> ets:lookup(TabId, K) end,
